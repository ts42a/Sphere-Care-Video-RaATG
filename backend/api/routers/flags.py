...
def get_flag_stats(
    ...
    manual_flags = (
        db.query(func.count(models.Flag.id))
        .filter(models.Flag.admin_id == admin_id, models.Flag.source == "Staff")
        .scalar()
        or 0
    )
    ...
    pending_review = (
        db.query(func.count(models.Flag.id))
        .filter(models.Flag.admin_id == admin_id, models.Flag.status == "Pending Review")
        .scalar()
        or 0
    )
    ...
    resolved = (
        db.query(func.count(models.Flag.id))
        .filter(models.Flag.admin_id == admin_id, models.Flag.status == "Resolved")
        .scalar()
        or 0
    )
    ...
    total = (
        db.query(func.count(models.Flag.id))
        .filter(models.Flag.admin_id == admin_id)
        .scalar()
        or 0
    )
...
def get_flag(
    ...
    if int(flag.admin_id) != int(admin_id):
        raise HTTPException(status_code=404, detail="Flag not found.")
    return _fmt(flag)
