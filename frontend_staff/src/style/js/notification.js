...
updateClock();
setInterval(() => { document.getElementById('topbar-time').textContent = new Date().toLocaleTimeString('en-AU', { hour:'numeric', minute:'2-digit', hour12:true }).toUpperCase(); }, 30000);
...
async function loadData() {
  try {
    ...
    const weekBookings = bookings.filter(b => weekDates.includes(b.appointment_date)); // ← fixed
    allNotifs = weekBookings.map(b => {
      ...
      if (b.booking_type.toLowerCase().includes('medication')) {
        iconType='meds'; title=`Medication: ${b.booking_type}`; desc=`${resName}'s ${b.booking_type.toLowerCase()} requires attention. ${b.doctor_name} · ${b.start_time}`;
      } else {
        title=`${b.booking_type} – ${resName}`; desc=`${b.doctor_name} · ${b.appointment_date} at ${b.start_time}`;
      }
      return { id:b.id, title, desc, time:timeAgo(b.appointment_date, b.start_time), rawTime:b.start_time, date:b.appointment_date, iconType, type, status:b.status, unread:b.status!=='completed', booking:b };
    });
    allNotifs.sort((a,b) => { if(a.date===fmtDate(now)&&b.date!==fmtDate(now))return -1; if(b.date===fmtDate(now)&&a.date!==fmtDate(now))return 1; return a.date.localeCompare(b.date)||a.rawTime.localeCompare(b.rawTime); });
    renderNotifs();
    loadPriorityAlerts();
  } catch(e) {
    console.warn('API error:', e);
    document.getElementById('notif-list').innerHTML = '<div class="empty-state">Could not load notifications.<br>Make sure the API server is running.</div>';
  }
}
...
function renderPriority(alerts) {
  ...
  el.innerHTML = alerts.slice(0,4).map(a => {
    const isCrit=a.alert_type==='critical', ago=a.created_at?timeAgoFromISO(a.created_at):'';
    const iconSvg=isCrit?`<svg class="priority-icon" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`:`<svg class="priority-icon" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`;
    return `<div class="priority-item ${a.alert_type}">${iconSvg}<div class="priority-text"><div class="priority-title">${a.title}</div><div class="priority-body">${a.description||a.body||a.message||''}</div>${ago?`<div class="priority-time">${ago}</div>`:''}</div></div>`;
  }).join('');
}

function timeAgoFromISO(iso) {
  try { const dt=new Date(iso),diffMins=Math.round((now-dt)/60000); if(diffMins<1)return'just now'; if(diffMins<60)return`${diffMins} mins ago`; return`${Math.round(diffMins/60)} hrs ago`; } catch{return'';}
}
...
(function(){
  ...
  function connect(){
    ...
    ws.onmessage=function(e){
      ...
      // ai_alert — from AiInsight (body) or Alert (message) — backend maps to description
      if(msg.type==='ai_alert'){
        var a=msg.alert, existing=document.getElementById('priority-list');
        if(existing){
          var isCrit=a.alert_type==='critical';
          var iconStr=isCrit?'<svg class="priority-icon" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>':'<svg class="priority-icon" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>';
      ...
      // booking_created — uses appointment_date / start_time
      if(msg.type==='booking_created'){
        ...
        if(!allNotifs.find(function(x){return x.id===newN.id;})){allNotifs.unshift(newN);renderNotifs();}
      }

      if(msg.type==='booking_updated'){
        var u=msg.booking,n=allNotifs.find(function(x){return x.id===u.id;});
        if(n){n.status=u.status;n.unread=u.status!=='completed';renderNotifs();}
      }

      if(msg.type==='booking_deleted'){allNotifs=allNotifs.filter(function(x){return x.id!==msg.booking_id;});renderNotifs();}
    };
  }
  connect();
})();