...
A bi-directional GRU or LSTM with a per-feature variable-selection gate (the "GRN"
style block) reads the per-sample feature vectors as a single sequence
and emits, per timestep:
...
def run_inference(
    ...
    *,
    ...
    blend_with_rules: float = 0.7,
    out_path: Path | None = None,
) -> Path:
    if not (0.0 <= blend_with_rules <= 1.0):
        raise SystemExit("--blend-rules must be in [0, 1]")
    ...
    if weights_loaded:
        # trust the trained model more
        a = blend_with_rules * 0.4
    else:
        # untrained model -> trust rules far more
        a = blend_with_rules
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Blend coefficient between model and rule logits when no weights "
        "are loaded (default 0.7 -> rule-dominated). When weights are loaded "
        "this is automatically scaled down to 0.4 of the requested value.",
    )
