...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SRM on Motion/Static split dataset.")
...
def main() -> None:
    ...
    if not train_path.exists() or not val_path.exists():
        raise FileNotFoundError(f"Missing split files in {data_dir}. Need train.jsonl and val.jsonl")

    train_rows = load_jsonl(train_path)
    val_rows = load_jsonl(val_path)

    vocab_dict = build_vocab(train_rows, min_freq=args.min_freq)
    vocab_payload = {
