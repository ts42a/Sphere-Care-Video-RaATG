...
def run_interactive(model: GRUSeq2Seq, vocab: Vocab, device: torch.device, max_len: int) -> None:
    print("SRM interactive mode. Type 'exit' to quit.")
    while True:
        ...
        if user_in.lower() in {"exit", "quit"}:
            break
        out = refine_text(model, vocab, user_in, device, max_len=max_len)
        print(f"clean> {out}")


def main() -> None:
    parser = argparse.ArgumentParser(description="SRM inference CLI.")
    parser.add_argument("--vocab_path", type=str, required=True)
    parser.add_argument("--checkpoint_path", type=str, required=True)
    parser.add_argument("--text", type=str, default="")
    parser.add_argument("--max_len", type=int, default=30)
    parser.add_argument("--json", action="store_true", help="Print JSON output for single text mode.")
    ...
    run_interactive(model, vocab, device, max_len=args.max_len)
