...
def _infer_src_fps_for_fi_from_step1(step1: dict[str, Any] | None) -> float:
    """Step-1 timestamps are derived from some internal fps (often 30 when
    OpenCV reports bogus WebM fps). To keep `source_frame_index` keys
    consistent across Step 2 + Step 3, we infer that fps from Step-1
    frames."""
    if not step1:
        return 30.0
    ...
    for f in step1.get("frames") or []:
        try:
            ...
            continue
        if fi > 0 and ts > 0.0:
            # ts_sec ≈ src_index / src_fps_used_for_sampling
...
def _gather_window_webm_time_based(
    cap,
    *,
    anchor_ts_sec: float,
    window_sec: float,
    ...
    """WebM-safe version of `_gather_window`.

    For WebM on Windows, `CAP_PROP_FPS` / `CAP_PROP_FRAME_COUNT` and
    `CAP_PROP_POS_FRAMES` can be unreliable, which previously caused
    empty candidate lists and `emitted=0`.

    We seek by time (`CAP_PROP_POS_MSEC`) and we assign stable `fi` values
    by converting timestamps to `src_fps_for_fi` frame indices."""
    start_ts = max(0.0, float(anchor_ts_sec) - float(window_sec))
    end_ts = float(anchor_ts_sec) + float(window_sec)

    # Derive the starting "source frame index" for stable `fi` keys.
    ...
    while True:
        ...
        # Use a time-based stop if POS_MSEC is available.
    ...
    # Fallback: POS_MSEC seeks can be unreliable on WebM. If we got no
    # frames, reset to the beginning and skip forward sequentially.
    ...
    while True:
        ...
        pos_msec = float(cap.get(cv2.CAP_PROP_POS_MSEC) or 0.0)
        cur_ts = pos_msec / 1000.0 if pos_msec > 0 else None
        if cur_ts is not None and cur_ts < start_ts - 0.05:
            continue
        if cur_ts is not None and cur_ts > end_ts + 0.05:
            break
...
def run(
    ...
    *,
    ...
    if not cap.isOpened():
        ...
        return 1
    # WebM often reports bogus FPS/frame_count in OpenCV on Windows.
    # We use a derived fps only for stable `fi` <-> `ts_sec` mapping.
    src_fps = _infer_src_fps_for_fi_from_step1(step1) if video_is_webm else (float(cap.get(cv2.CAP_PROP_FPS) or 0.0) or 30.0)
    ...
    # WebM on Windows: seeking fails (CAP_PROP_POS_MSEC / CAP_PROP_POS_FRAMES
    # then read returns `ok=False`). Pre-decode the minimal set of frames we
    # need for all anchor windows, then gather candidates from this cache.
    frames_by_idx: dict[int, np.ndarray] | None = None
    frame_w: int | None = None
    frame_h: int | None = None

    if video_is_webm:
        # half_window is in "src frames" because `src_fps` is our mapping fps.
        half_window_frames = int(half_window)
        anchor_indices: list[int] = []
        for fmeta in person_frames:
            sample_ts = float(fmeta.get("ts_sec", 0.0))
            anchor_indices.append(
                int(fmeta.get("src_index") or int(round(sample_ts * src_fps)))
            )
        ...
        # Decode sequentially from the start and cache only needed indices.
        src_idx = 0
        ...
        if frame_w is None or frame_h is None:
            _emit_empty_stub("webm_predecode_failed")
            return 0
    ...
    for i, fmeta in enumerate(person_frames, start=1):
        ...
        if person_box_raw is not None:
            if video_is_webm:
                # All frames are same resolution; no need to probe seek.
        ...
        if video_is_webm:
            ...
            half_window_frames = int(half_window)
            lo = max(0, anchor_idx - half_window_frames)
            hi = anchor_idx + half_window_frames
            ...
            candidates.sort(key=lambda r: r["sharpness"], reverse=True)
        else:
